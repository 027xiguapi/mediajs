...
describe(`basic interop test ${browserA} => ${browserB}`, function () {
	...
	before(async () => {
		...
		drivers = [buildDriver(browserA, options), buildDriver(browserB, options)];
	...
	it("establishes a connection", async () => {
		...
		await steps.step(
			drivers,
			(d) => d.get("https://webrtc.github.io/samples/emptypage.html"),
			"Empty page loaded",
		);
		await steps.step(
			clients,
			(client) => client.connection.create(),
			"Created RTCPeerConnection",
		);
		await steps.step(
			clients,
			async (client) => {
				const stream = await client.mediaDevices.getUserMedia({
					audio: true,
					video: true,
				});
				return Promise.all(
		...
		const offerWithCandidates =
			await clients[0].connection.setLocalDescription();
		await clients[1].connection.setRemoteDescription(offerWithCandidates);
		const answerWithCandidates =
			await clients[1].connection.setLocalDescription();
		await clients[0].connection.setRemoteDescription(answerWithCandidates);

		await steps.step(
			drivers,
			(d) => steps.waitNVideosExist(d, 1),
			"Video elements exist",
		);
		await steps.step(
			drivers,
			steps.waitAllVideosHaveEnoughData,
			"Video elements have enough data",
		);
	}).timeout(30000);
}).timeout(90000);
