...
class MediaDevices {
	...
	getUserMedia(constraints) {
		return this.driver
			.executeAsyncScript((constraints) => {
...
class PeerConnection {
	...
	createOffer(offerOptions) {
		return this.driver.executeAsyncScript((offerOptions) => {
			...
			pc.createOffer(offerOptions).then(callback, callback);
		}, offerOptions);
	}
	createAnswer() {
		return this.driver.executeAsyncScript(() => {
			...
			pc.createAnswer().then(callback, callback);
		});
	}
	...
	setLocalDescription(desc) {
		return this.driver.executeAsyncScript((desc) => {
			...
			pc.setLocalDescription(desc).catch(callback);
		}, desc);
	}
